...
# The HTML Dashboard - Pure Vanilla JS
# No React, No Babel, No "Unexpected Token" errors.
DASHBOARD_HTML = """
<!DOCTYPE html>
<html lang="en">
<head>
    ...
    <title>Network Pulse v2.7</title>
...
<body class="p-4 md:p-8">
        ...
        <div class="flex justify-between items-center bg-slate-900/60 p-5 rounded-2xl border border-white/10 shadow-2xl">
            <div>
                <h1 class="text-2xl font-black tracking-tighter uppercase italic text-blue-500">
                    Network Pulse <span class="text-white opacity-40 text-xs not-italic ml-2 tracking-normal font-medium">v2.7 Stable</span>
                </h1>
                <p class="text-[10px] text-slate-500 font-bold uppercase tracking-[0.2em] mt-1">Direct Engine: Engine Stable</p>
            </div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div class="space-y-4">
                <div class="glass rounded-3xl p-6 border-blue-500/20">
                    ...
                    <p id="stat-ssid" class="text-[10px] text-slate-500 truncate mt-2 font-mono">Scanning...</p>
                </div>
            ...
            <div class="lg:col-span-2 space-y-6">
                <div class="glass rounded-3xl p-6 border-white/5 shadow-inner">
                    <div class="flex justify-between items-center mb-6">
                        <h3 class="text-xs font-black uppercase tracking-widest text-slate-400">Response History</h3>
                    ...
                    <div id="main-chart" class="h-48 grid-container border-b border-white/5">
                        <!-- Bars injected by JS -->
                    </div>
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div class="glass rounded-3xl p-6">
                        <h3 class="text-[10px] font-black uppercase tracking-widest text-slate-500 mb-4">Signal Stability %</h3>
                        <div id="signal-chart" class="h-24 grid-container"></div>
                    </div>
                    <div class="glass rounded-3xl p-6 border-yellow-500/10">
                        <h3 class="text-[10px] font-black uppercase tracking-widest text-yellow-500/50 mb-4">ISP Overhead</h3>
                        <div id="diff-chart" class="h-24 grid-container"></div>
                    </div>
                </div>
            </div>
        </div>
    ...
    <script>
        ...
        // Pre-create the columns for the charts
        function initCharts() {
            const containers = {
                'main-chart': true,
                'signal-chart': false,
            ...
            for (const [id, isDouble] of Object.entries(containers)) {
                ...
                for (let i = 0; i < MAX_DATA; i++) {
                    ...
                    if (isDouble) {
                        col.innerHTML = `
                            <div class="bar bg-purple-500/40" id="web-${id}-${i}" style="height:0%"></div>
                            <div class="bar bg-emerald-500/60" id="loc-${id}-${i}" style="height:0%"></div>
                        `;
                    } else {
                        const color = id === 'signal-chart' ? 'bg-blue-500/30' : 'bg-yellow-500/40';
                        col.innerHTML = `<div class="bar ${color}" id="val-${id}-${i}" style="height:0%"></div>`;
                    }
                    el.appendChild(col);
                }
            }
...
if __name__ == '__main__':
    print("--- NETWORK MONITOR v2.7 READY ---")
